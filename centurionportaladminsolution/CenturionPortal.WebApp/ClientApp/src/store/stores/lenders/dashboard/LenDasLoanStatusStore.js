"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.reducer = exports.actions = void 0;
var LoanStatusAction = require("../../../actions/lenders/dashboard/LenDasLoanStatusAction");
exports.actions = LoanStatusAction.actions;
exports.reducer = function (state, incomingAction) {
    if (state === undefined) {
        return {
            loading: false,
            update: false,
            collapse: false,
            fullScreen: false,
            loanStatus: [],
            categoriesStatus: [],
            seriesStatus: []
        };
    }
    var action = incomingAction;
    switch (action.type) {
        case 'LOADING_LOAN_STATUS':
            return __assign(__assign({}, state), { loading: action.loading });
        case 'FETCHED_LOAN_STATUS':
            return __assign(__assign({}, state), { loading: false, update: true, loanStatus: action.loanStatus, categoriesStatus: action.categoriesStatus, seriesStatus: action.seriesStatus });
        case 'UPDATED_LOAN_STATUS':
            return __assign(__assign({}, state), { update: false });
        case 'COLLAPSE_LOAN_STATUS':
            return __assign(__assign({}, state), { collapse: action.active });
        case 'FULL_SCREEN_LOAN_STATUS':
            return __assign(__assign({}, state), { fullScreen: action.active });
        default: return state;
    }
};
//# sourceMappingURL=LenDasLoanStatusStore.js.map