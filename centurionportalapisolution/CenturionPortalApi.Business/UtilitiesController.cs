...
namespace CenturionPortalApi.Business
{
    public static class UtilitiesController
    {
        ...
        public    class SelectSomeProperties<T>
        {

            private readonly Type Type;
            private readonly PropertyInfo[] PropertyInfo;

            public SelectSomeProperties(  params string[] propertyName)
            {
                this.Type = typeof(T);

                propertyName = propertyName.Select(x => x.ToUpper().Trim()).ToArray();

                this.PropertyInfo = this.Type.GetProperties().Where(x => propertyName.Contains(x.Name.ToUpper())).ToArray();
            }
            ...
            public   T Select(T obj)
            {
                ...
                foreach (var p in this.PropertyInfo)// type.GetProperties().Where(x => propertyName.Contains(x.Name)))
                {
        ...
        private static IDictionary<string, string> _mappingsExtension = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase)
        {
        ...
        public static DataSourceRequest Convert_StringToDataSourRequest(string dataSourceRequest)
        {
        ...
        /// <summary>
        /// Retorna un objeto de tipo CenturionPortalApi.DataBase.Models.ELSUser 
        /// </summary>
        /// <param name="UserLogin_"></param>
        /// <returns></returns>
        public static ELSUser GetELSUser(System.Security.Claims.ClaimsPrincipal UserLogin_)
        {
            DataBase.Models.ELSUser ELSUser_ = null;
            if (UserLogin_ != null && UserLogin_.Claims.Count() > 0)
            {
                ELSUser_ =
                    new DataBase.Models.ELSUser
                    {
        ...
        public static bool ValidateUserByLoanAccount(string userUid, string loanAccount)
        {
            return ELSServiceMapController.GetMatchUserAndLoanAccount(userUid, loanAccount).Count() == 0 ? false : true;
        }

        #region Constants
        //La contraseña debe tener al menos 6 caracteres, al menos una minúscula, al menos una mayúscula, al menos un dígito y al menos un caracter no alfanumérico.
        //string formato = @"^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[\u0021-\u002b\u003c-\u0040])\S{6,}$";

        ///La contraseña debe tener al menos 6 caracteres, al menos una minúscula, al menos una mayúscula, al menos un dígito y al menos un caracter no alfanumérico.
        public const string RegExStrongPassword = @"^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{6,})";
        #endregion

        #region Images
        public static bool IsImageByPath(string file)
        {
            ...
            return formats.Any(item => file.EndsWith(item, StringComparison.OrdinalIgnoreCase));
        }
        ...
        public static string GetMimeType(string fileName)
        {
            ...
            return _mappingsExtension.TryGetValue(extension, out mime) ? mime : "application/octet-stream";
        }

        public static string getUrlImage(string photo)
        {
            string pathPublic = AppSetting("ImagePath:Public");
            string pathThumbOriginal = AppSetting("ImagePath:ThumbOriginal");
            string pathComplete = Path.Combine(Directory.GetCurrentDirectory(), pathPublic);
            pathComplete = Path.Combine(pathComplete, pathThumbOriginal);

            if (!string.IsNullOrEmpty(photo) && File.Exists(Path.Combine(pathComplete, photo)))
                photo = Path.Combine(pathThumbOriginal, photo).Replace("\\", "/");
            else
                photo = AppSetting("ImagePath:NoLogo").Replace("\\", "/");
